...
void print_all_volt_curr()
{
  ...
  float   current,voltage, temp;
        ...
        u8g2.setFont(u8g2_font_6x10_tr);
        u8g2.setCursor(0,10);
    voltage = pmbus_readVin(ps_i2c_address);
    current = pmbus_readIin(ps_i2c_address);
        u8g2.print(F("IN: "));
    ...
    voltage = pmbus_readVout(ps_i2c_address);
    current = pmbus_readIout(ps_i2c_address);
        u8g2.setCursor(0,20);
        u8g2.print(F("OT: "));
        ...
        u8g2.print(F("C="));
        u8g2.print(current, 2);
//          temp = pmbus_readOtemp(ps_i2c_address);           //temp sensor 0x8D  
//          voltage = pmbus_readItemp(ps_i2c_address);        //temp sensor 0x8E  
//          current = pmbus_readtemp3(ps_i2c_address);        //temp sensor 0x8F  
//          u8g2.setCursor(0,30);
//          u8g2.print(F("8D "));
//          u8g2.print(temp, 0);
...
//          temp = pmbus_readFanSpeed1(ps_i2c_address);
//           u8g2.setCursor(0,40);
//           u8g2.print(F("Fan1 Speed  "));
//           u8g2.print(temp, 1);
          w_val = pmbus_readStatusWord(ps_i2c_address);
          u8g2.setCursor(0,50);
          u8g2.print(F("STATUS WORD 0x"));
          u8g2.print(w_val, HEX);          
     voltage = pmbus_readPin(ps_i2c_address);
     current = pmbus_readPout(ps_i2c_address);
        u8g2.setCursor(0,60);
        u8g2.print(F("Pin="));
        u8g2.print(voltage, 1);
...
void print_all_sensors()
  {
      ...
      u8g2.setFont(u8g2_font_6x10_tr);    
      u8g2.setCursor(0,10);
        temp = pmbus_readOtemp(ps_i2c_address);           //temp sensor 0x8D  Inlet Air
          u8g2.print(F("Inlet Air  Temp "));
          u8g2.print(temp, 1);
          Serial.print(F("Inlet Air Temp "));
          Serial.println(temp, 1);
           temp = pmbus_readItemp(ps_i2c_address);            //0x8E Hotspot1 Sensor located on secondary side of DC/DC stage
            u8g2.setCursor(0,20);
            u8g2.print(F("DC/DC stag Temp "));
            u8g2.print(temp, 1);
            Serial.print(F("DC/DC stage Temp "));
            Serial.println(temp, 1);
              temp = pmbus_readMtemp(ps_i2c_address);          // 0x8F Hotspot2 Sensor located on primary heat sink
                 u8g2.setCursor(0,30);
...
//                      temp = pmbus_readtemp2(ps_i2c_address);
                        ...
                        Serial.println(temp, 1);
                          fspeed = pmbus_readFanSpeed1(ps_i2c_address);
                            u8g2.setCursor(0,60);
                            u8g2.print(F("Fan1 Speed  "));
                            u8g2.print(fspeed, 1);
                            u8g2.sendBuffer();
...
float readeout(){
 ...
 // unsigned int m,b,r;
  ...
  u8g2.setFontMode(1);
  u8g2.setFont(u8g2_font_6x10_tr);
//  smbus_readBlock(ps_i2c_address, 0x30, data, 5);
//        u8g2.clearBuffer();
//        u8g2.setCursor(0,10);
//        u8g2.print(F("Data "));
