void checkSerial(){
   ...
   if(key == 2) serial_smbus_commands();                //Button D click   
      ...
      case 8:
       ...
       case 9:
        ...
        for(int i = 0; i < sm.writebytes; i++) {
          ...
          d[3*i] = ' ';
          d[3*i + 1] = hex_table[sm.datablock[i] >> 4];
          d[3*i + 2] = hex_table[sm.datablock[i] & 0x0f];     
        }
        d[3*sm.writebytes] = '\0'; 
...
//          if(mqttflag) client.publish("rrh/pmbus/set/rinfo", "No Readback");
        }
        delay(5);
        break;
